...
//vue运行在浏览器中，所以需要考虑性能
//每次数据的更新 -> 虚拟Dom的生成(模板解析的行为) -> 因此将经常使用的字符串与算法进行缓存
...
//在模板中常常会用‘指令’，在vue中一个xxx-xxx-xxx的形式出现的属性
//每次数据的更新都可能会带来指令的解析，所以解析就是字符串处理，一般会消耗一定的性能。
...
//3.再遍历b中的成员，判断是否每一个b中的成员都在a中，并且与b中的对应成员相等
//4.如果成员是引用类型，递归。
  ...
  if (isObjectA && isObjectB) {
    try {
      ...
      if (isArrayA && isArrayB) {
        ...
        return a.getTime() === b.getTime()//单独处理Date类型，时间cuo应该是一样的
...
　
//让一个事件（一个函数）只允许调用一次
//在vue中有函数方法（on,off等，once）,once事件就是这个思路
/**
